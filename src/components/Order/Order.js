...
function Order() {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const response = await axios.get(`https://uppercase-back-1eec3e8a2cf1.herokuapp.com/order/${basketId}`, {
  ...
  const handlePaymentAccepted = async () => {
    try {
      const response = await axios.post(`https://uppercase-back-1eec3e8a2cf1.herokuapp.com/createInvoice`, { basketId: basketId , basketEmail: orderDetails.email}, { withCredentials: true });
